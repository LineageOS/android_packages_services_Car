...
public class CarPropertyManagerTest extends MockedCarTestBase {
    ...
    @RequiresFlagsEnabled(FLAG_CAR_PROPERTY_STATUS_DETAILED_NOT_AVAILABLE)
    @Test
    public void testOnChangeEvent_withVendorPropertyStatus_withPermission() throws Exception {
        createNewManagerWithPermissions(
                new String[] {Car.PERMISSION_READ_PROPERTY_VENDOR_STATUS}, new String[] {});
        // Two initial values because CUSTOM_SEAT_INT_PROP_1 has 2 areaIds.
        TestCallback callback = new TestCallback(
                /* initValueCount= */ 2, /* changeEventCount= */ 1, /* errorEventCount= */ 0);
        mManager.registerCallback(callback, CUSTOM_SEAT_INT_PROP_1, /* sampleRate= */ 0);
    ...
    public void testOnChangeEvent_withVendorPropertyStatus_withoutPermission() throws Exception {
        createNewManagerWithPermissions(new String[] {}, new String[] {
    ...
    // Create a new Car instance if permissions need to be read during object initialization.
    private void createNewManagerWithPermissions(
            String[] allowedPermissions, String[] deniedPermissions) {
        ((MockedCarTestContext) getContext()).setAllowedPermissions(allowedPermissions);
        ((MockedCarTestContext) getContext()).setDeniedPermissions(deniedPermissions);
        Car car = createNewCar();
        mManager = car.getCarManager(CarPropertyManager.class);
