...
public class CarDisplayAreaTouchHandler {
    ...
    public void setTitleBarViewBounds(Rect titleBarBounds) {
        mTitleBarBounds = titleBarBounds;
    }

    public void setTrackingBounds(Rect trackingBounds) {
        mTrackingBounds = trackingBounds;
    }
    ...
    private void onMotionEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (isStartFromTrackingBounds()) {
                    if (mOnClickDisplayAreaListener != null) {
    ...
    private boolean isStartFromTrackingBounds() {
        return mTrackingBounds
                != null && mStartX >= mTrackingBounds.left && mStartX <= mTrackingBounds.right
                && mStartY >= mTrackingBounds.top && mStartY <= mTrackingBounds.bottom;
    }
    ...
    /**
     * Callback invoked when a user clicks start from tracking bounds.
     */
    interface OnClickDisplayAreaListener {

        /**
         * Called for ACTION_UP touch events start from tracking bounds.
         */
