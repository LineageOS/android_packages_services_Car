...
const char kDemoGraphName[] = "Object Detection Graph Demo";
...
ndk::ScopedAStatus ObjectDetector::init(std::function<void(bool, std::string)>&& cb,
    ...
    if (mUseDemoGraph) {
        status = queryService->getPipeRunner(kDemoGraphName, mClientInfo, &mPipeRunner);
    } else {
