...
EGLDisplay   GlRenderer::sGLDisplay;
EGLSurface   GlRenderer::sGLSurface;
EGLContext   GlRenderer::sGLContext;
GLuint       GlRenderer::sFrameBuffer;
GLuint       GlRenderer::sColorBuffer;
GLuint       GlRenderer::sDepthBuffer;
GLuint       GlRenderer::sTextureId;
EGLImageKHR  GlRenderer::sKHRimage;

const char* GlRenderer::getEGLError(void) {
    switch (eglGetError()) {
        case EGL_SUCCESS:
            return "EGL_SUCCESS";
...
const std::string GlRenderer::getGLFramebufferError(void) {
...
bool GlRenderer::initialize() {
    ...
    if (!mDisplayHandler->startDisplay()) {
        LOG(ERROR) << "Failed to startDisplay";
        return false;
    }
...
bool GlRenderer::attachNewRenderTarget() {
    // Get a new display buffer target.
    sp<GraphicBuffer> pGfxBuffer;
    if (!mDisplayHandler->getNewDisplayBuffer(&pGfxBuffer)) {
        LOG(ERROR) << "Failed to obtain new display buffer.";
        return false;
    }
    ...
    // Set the viewport
    glViewport(0, 0, pGfxBuffer->getWidth(), pGfxBuffer->getHeight());
...
bool GlRenderer::detachAndDisplayCurrRenderTarget() {
    ...
    if (!mDisplayHandler->displayCurrentBuffer()) {
        LOG(ERROR) << "Failed to displayCurrentBuffer.";
        return false;
    }
    return true;
}

GlRenderer::GlRenderer(sp<DisplayHandler> displayHandler) :
    mDisplayHandler(displayHandler) {
...
bool GlRenderer::makeContextCurrent() {
    ...
    LOG(DEBUG) << "Context current is current for GlRenderer.";
    return true;
}

bool GlRenderer::clearContext() {
    ...
    LOG(DEBUG) << "Context is cleared for GlRenderer.";
    return true;
}
