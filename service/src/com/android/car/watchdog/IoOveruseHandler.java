...
public final class IoOveruseHandler {
    ...
    public static final String USER_PACKAGE_SEPARATOR = ":";
    public static final String PACKAGES_DISABLED_ON_RESOURCE_OVERUSE_SEPARATOR = ";";
    public static final long MAX_DAEMON_CONNECTION_WAIT_TIME_MILLS = 3_000;
    ...
    public void init() {
        ...
        mServiceHandler.post(() -> {
            ...
            // atoms.
            Trace.beginSection("IoOveruseHandler.init-async-atomSetUp");
    ...
    public void onDaemonConnectionChange(boolean isConnected) {
        Trace.beginSection("IoOveruseHandler.onDaemonConnectionChange(isConnected=" + isConnected
                + ")");
    ...
    @UxStateType
    public int getCurrentUxState() {
        synchronized (mLock) {
            return mCurrentUxState;
        }
    }
    ...
    public void setKillablePackageAsUser(String packageName, UserHandle userHandle,
        ...
        Trace.beginSection("IoOveruseHandler.setKillablePackageAsUser(package= " + packageName
                + ", userId=" + userId + ", isKillable= " + isKillable + ")");
        try {
            String genericPackageName = mPackageInfoHandler.getNameForUserPackage(packageName,
                    userId);
            if (genericPackageName == null) {
                throw new IllegalArgumentException("Package '" + packageName + "' not found");
            }
            ...
            synchronized (mLock) {
                // When the queried package is not cached in {@link mUsageByUserPackage}, the set
                // API will update the killable state even when the package should never be killed.
                // But the get API will return the correct killable state. This behavior is
                // tolerable because in production the set API should be called only after the get
                // API. For instance, when this case happens by mistake and the package overuses
                // resource between the set and the get API calls, the daemon will provide correct
                // killable state when pushing the latest stats. Ergo, the invalid killable state
    ...
    private void setPackageKillableStateForAllUsers(String packageName, boolean isKillable) {
        Trace.beginSection("IoOveruseHandler.setPackageKillableStateForAllUsers(package="
                + packageName + ", isKillable=" + isKillable + ")");
        try {
            ...
            // Enabling user packages requires accessing package manager which requires making
            // binder calls. Binder calls should not be made while holding a lock, given it might
            // lead to deadlock. Hence, enabling packages after the synchronized block.
            if (!isKillable) {
    ...
    private void latestIoOveruseStatsInternal(List<PackageIoOveruseStats> packageIoOveruseStats) {
        Trace.beginSection("IoOveruseHandler.latestIoOveruseStatsInternal");
    ...
    public void asyncFetchTodayIoUsageStats() {
        mServiceHandler.post(() -> {
            Trace.beginSection("IoOveruseHandler.asyncFetchTodayIoUsageStats");
    ...
    public List<UserPackageIoUsageStats> getTodayIoUsageStats() {
        Trace.beginSection("IoOveruseHandler.getTodayIoUsageStats");
    ...
    public void processActionPackageChanged(Intent intent) {
        ...
        Trace.beginSection("IoOveruseHandler.processPackageChangedIntent(package=" + packageName
                + ", userId=" + userId + ")");
        try {
            try {
    ...
    public boolean disablePackageForUser(String packageName, @UserIdInt int userId) {
        Trace.beginSection("IoOveruseHandler.disablePackageForUser(package=" + packageName
                + ", userId=" + userId + ")");
        try {
            synchronized (mLock) {
                ArraySet<String> disabledPackages = mDisabledUserPackagesByUserId.get(userId);
            ...
            try {
                ...
                switch (currentEnabledState) {
                    ...
                    case COMPONENT_ENABLED_STATE_DISABLED_UNTIL_USED:
                        Slogf.w(TAG, "Unable to disable application for user %d, package '%s' "
                                        + "as the current enabled state is %s", userId, packageName,
    ...
    public void writeMetadataFile() {
        ...
        Trace.beginSection("IoOveruseHandler.writeMetadataFile");
    ...
    private void fetchAndSyncResourceOveruseConfigurations() {
        Trace.beginSection("IoOveruseHandler.fetchAndSyncResourceOveruseConfigurations");
        try {
            List<android.automotive.watchdog.internal.ResourceOveruseConfiguration> internalConfigs;
            try {
                internalConfigs = mCarWatchdogDaemonHelper.getResourceOveruseConfigurations();
    ...
    public void writeToDatabase() {
        ...
        Trace.beginSection("IoOveruseHandler.writeToDatabase");
        try {
    ...
    private boolean writeStats(List<WatchdogStorage.IoUsageStatsEntry> ioUsageStatsEntries,
        ...
        Trace.beginSection("IoOveruseHandler.writeStats");
        int result = WatchdogStorage.FAILED_TRANSACTION;
        try {
            // Forgive historical overuses before writing the latest stats to disk to avoid
            // forgiving the latest stats when the write is triggered after date change.
            ...
            result = mWatchdogStorage.saveIoUsageStats(ioUsageStatsEntries);
    ...
    private int setResourceOveruseConfigurationsInternal(
        ...
        Trace.beginSection("IoOveruseHandler.setResourceOveruseConfigurationsInternal");
        try {
    ...
    private void performOveruseHandlingLocked() {
        ...
        Trace.beginSection("IoOveruseHandler.performOveruseHandlingLocked");
    ...
    private void notifyUserOnOveruse() {
        Trace.beginSection("IoOveruseHandler.notifyUserOnOveruse");
        ...
        synchronized (mLock) {
            ...
            for (int i = mUserNotifiablePackages.size() - 1; i >= 0; i--) {
                ...
                if (usage.isSharedPackage()) {
                    packages.addAll(mPackageInfoHandler.getPackagesForUid(usage.getUid(),
                            usage.genericPackageName));
                } else {
                    packages.add(usage.genericPackageName);
                }
            }
        }
        Slogf.i(TAG, "Sending notifications for %d packages", packages.size());
        List<String> packagesWithNotifications = mIoOveruseHelper.sendUserNotifications(
                currentUserId, packages);
        synchronized (mLock) {
            for (int i = 0; i < packagesWithNotifications.size(); i++) {
                String packageName = packagesWithNotifications.get(i);
                String uniqueId = getUserPackageUniqueId(currentUserId, packageName);
                mUserNotifiablePackages.remove(uniqueId);
            }
        }
        Trace.endSection();
    }
    ...
    private void enablePackageForUser(int uid, String genericPackageName) {
        ...
        Trace.beginSection("IoOveruseHandler.enablePackageForUser(genericPackage="
                + genericPackageName + ", userId=" + userId + ")");
        List<String> packages;
    ...
    interface IoOveruseHelper {
        ...
        /**
         * Send user notification on disabling apps due to I/O overuse.
         *
         * Implement this callback to send notifications. The callback must verify whether
         * the current user is the same as the user whose app is disabled before showing
         * notification.
         *
         * The callback must return a list of packages with successful notification. This list will
         * be used to update the internal cache at
         * {@link IoOveruseHandler.mUserNotifiablePackages}.
         *
         * @param userId User ID of the packages that were disabled.
         * @param packages Disabled packages for which the user must be notified.
         * @return List of packages for which user notifications were successfully sent.
         */
        @Nullable
